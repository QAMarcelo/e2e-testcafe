...
test
    ...
    ('Pick to Clean-Manual', async t =>{
        /** VARIABLES  */
        let account = scenario.businessPartners![0];
        ...
        if(scenario.businessPartners)
            scenario.businessPartners = [account];
        await BackEnd.LoadAccount(account)
        //const account = 'Allocation Account';
        const accountDescription = scenario.businessPartners![0].Description!;
        ...
        await createOrder(account, orderNumber1, [{oQty: 10}], item1);
        // Create an order ( status in process ) with 2 entry lines, 1st with ordered and shipped 1ty = 4 and 2nd line with ordered and shipped qty =3
        // await DVU.ShippingOrders.Toolbar.Insert.Click();
        await createOrder(account, orderNumber2, [{oQty: 4}, {oQty: 3}], item1);
        ...
        //allocation g2 -AllocationLPN-04 - 2
        //Allocation G2 - AllocationLPN-04 - 2
        //Allocation G2-AllocationLPN-03-5
        //Allocation G2-AllocationLPN-03-1
